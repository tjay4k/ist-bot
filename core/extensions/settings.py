...
async def event_category_autocomplete(interaction: Interaction, current: str) -> list[app_commands.Choice[str]]:
    ...
    rows = await db.fetch(
        "SELECT DISTINCT event_category FROM action_log_events WHERE guild_id = $1", interaction.guild.id
    )
    return [
        app_commands.Choice(name=row["event_category"], value=row["event_category"])
        for row in rows
...
async def cog_autocomplete(interaction: Interaction, current: str) -> list[app_commands.Choice[str]]:
    ...
    dev_guild_ids = await interaction.client.get_service("db").fetch("SELECT guild_id FROM dev_guilds")
    ...
    return [app_commands.Choice(name=cog, value=cog) for cog in loaded if current.lower() in cog.lower()][:25]
...
class Settings(commands.Cog):
    ...
    settings = app_commands.Group(name="settings", description="Manage bot settings")
    action_log_group = app_commands.Group(name="actionlog", description="Manage action log settings", parent=settings)
    module_group = app_commands.Group(name="module", description="Manage modules", parent=settings)
    ...
    async def set_action_log_channel(
        self, interaction: discord.Interaction, category: str, event: str, channel: discord.TextChannel = None
    ):
        ...
        if channel:
            embed = create_embed(description=f"✅ Set `{category} → {event}` log channel to {channel.mention}")
        else:
            embed = create_embed(description=f"✅ Cleared log channel for `{category} → {event}`")
    ...
    async def list_cogs(self, interaction: discord.Interaction):
        ...
        rows = await self.db.fetch(
            "SELECT cog_name, enabled FROM guild_cog_config WHERE guild_id = $1", interaction.guild.id
        )
        disabled = {row["cog_name"] for row in rows if not row["enabled"]}

        cog_list = "\n".join(f"{'✅' if cog not in disabled else '❌'} `{cog}`" for cog in sorted(loaded))

        embed = create_embed(
            title="Cog Status", description=cog_list or "No Modules enabled", color=discord.Color.blurple()
        )
        await interaction.followup.send(embed=embed, ephemeral=True)
